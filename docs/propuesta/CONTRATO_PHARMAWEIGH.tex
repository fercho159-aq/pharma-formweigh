...
\begin{enumerate}
    \item PharmaWeigh incorpora funciones \textbf{orientadas} al control de procesos regulados: identificación de usuario por correo y contraseña, roles con permisos diferenciados, firma electrónica por fase restringida a los roles autorizados, bitácora de acciones inmutable a nivel de base de datos conforme al Anexo D.4, control de estado y caducidad de lotes y registro del peso objetivo contra el peso real dispensado.
...
Auditoría & Consulta de la bitácora con usuario, rol, acción, entidad, identificador, detalle, dirección IP y fecha, con filtros por acción y por usuario. La bitácora es de sólo inserción conforme al Anexo D.4. \\
...
\vspace{6pt}\needspace{4\baselineskip}\subseccion{A.6 Pruebas automatizadas entregadas}
\noindent Forman parte del entregable y se ejecutan con los comandos documentados en el manual de operación técnica:
\vspace{4pt}
\noindent
\begin{tabularx}{\linewidth}{@{}L{5.0cm} X@{}}
\toprule
163 pruebas unitarias & De las reglas de negocio, sin base de datos: tolerancias de pesaje, estados y caducidad de lote, avance de fases, cantidades y permisos por rol. \\
Prueba de punta a punta, 72 verificaciones & Contra la imagen de producción y una base de datos desechable. Cubre el camino completo --- alta de material, recepción de lote, aprobación por calidad, receta por fases, orden, pesaje, firma por fase y cierre en estado DISPENSADO --- y los intentos de burlarlo: sesión falsificada, tolerancia alterada desde el navegador, ráfaga de ocho pesajes simultáneos de la que se acepta uno solo, existencia que nunca queda negativa y bloqueo del acceso tras cinco intentos fallidos. \\
\bottomrule
\end{tabularx}

\vspace{8pt}
\begin{cajaAviso}[Fuera de alcance del Anexo A]
Validación del sistema (CSV, IQ/OQ/PQ) y certificación 21 CFR Part 11 o NOM-059; lectura automática del peso desde básculas; integraciones con ERP, LIMS o MES; pruebas automatizadas de interfaz en navegador; notificaciones por correo o mensajería; aplicaciones móviles nativas; y el hardware de planta. Ver cláusula \ref{cl:alcance} y Anexo C.5.
\end{cajaAviso}
...
Pruebas automatizadas de interfaz en navegador & \HR{\hOb} & \IM{\hOb} & --- \\
...
Bitácora & Inmutable en el motor de base de datos: disparadores de PostgreSQL rechazan \texttt{UPDATE}, \texttt{DELETE} y \texttt{TRUNCATE} sobre la tabla de auditoría. Cada asiento se escribe en la misma transacción que la operación que lo origina y registra la dirección IP. EL PRESTADOR hace constar el límite de esta medida: un superusuario del motor de base de datos podría desactivar los disparadores, y no existe encadenado criptográfico de los asientos ni copia en un tercero de confianza. \\
