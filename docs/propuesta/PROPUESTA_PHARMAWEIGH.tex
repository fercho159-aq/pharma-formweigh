...
\begin{itemize}[leftmargin=16pt, itemsep=4pt, topsep=4pt]
    ...
    \item Cada acción del sistema en una \textbf{bitácora} consultable con filtros por acción y por usuario, con la IP de quien la ejecutó. La base de datos rechaza cualquier intento de modificarla o borrarla.
...
Bitácora de auditoría & \textbf{Construida e inmutable en la base de datos.} Cada asiento se escribe en la misma transacción que la operación que lo origina y guarda la dirección IP. Tres disparadores de PostgreSQL rechazan \texttt{UPDATE}, \texttt{DELETE} y \texttt{TRUNCATE} sobre la tabla: no es que la aplicación no ofrezca el botón, es que el motor no lo permite. \textbf{Límite honesto:} un superusuario de la base de datos podría desactivar los disparadores; no hay encadenado por hash ni copia en un tercero. \\
...
Pruebas automatizadas & \textbf{Construidas.} \textbf{163 pruebas unitarias} de las reglas de negocio, sin base de datos: tolerancias, estados de lote, fases, cantidades y permisos por rol. Más una \textbf{prueba de punta a punta con 72 verificaciones} que corre contra la imagen de producción y cierra en verde. No hay pruebas de interfaz en navegador (ver sección 5.5). \\
...
\textbf{Pruebas de interfaz en navegador:} el juego actual cubre las reglas y la API; esto agrega el recorrido del operario en pantalla, en el equipo real de planta. & \HR{\hOb} & \IM{\hOb} & --- \\
