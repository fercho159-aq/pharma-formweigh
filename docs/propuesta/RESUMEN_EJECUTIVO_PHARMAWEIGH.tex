...
\midrule
Todo queda asentado & Peso objetivo contra peso real, lote usado, quién dispensó, quién firmó, desde qué IP y a qué hora. La bitácora es \textbf{inmutable en la base de datos}: el motor rechaza modificarla o borrarla. \\
\bottomrule
\end{tabularx}
\normalsize
...
El sistema está \textbf{construido y desplegado}, con siete roles diferenciados (admin, supervisor, operario, desarrollo, calidad, almacén y auditor), inventario con estados de lote y caducidad, recetas por fases con tolerancias, órdenes de producción, dispensado guiado, firma por fase, bitácora, generación de etiquetas de código de barras, respaldo diario y manuales por rol. Va con \textbf{163 pruebas automáticas} de las reglas de negocio y una \textbf{prueba de punta a punta de 72 verificaciones} que corre en verde contra la imagen de producción, incluidos los intentos de burlar el control: sesión falsificada, tolerancia alterada desde el navegador, doble clic en el pesaje y existencia negativa. Se puede revisar pantalla por pantalla hoy en \href{https://\demoURL}{\demoURL}.
\end{cajaInfo}
...
\noindent{\small\color{gristexto}\textbf{Opcionales}, con la misma tarifa: lectura directa desde báscula, validación del sistema (CSV, IQ/OQ/PQ), pruebas de interfaz en navegador, integración con ERP, notificaciones por correo y dominio propio. Importes en la sección 5.5 de la propuesta.}
