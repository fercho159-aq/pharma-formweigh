...
export default function InventarioPage() {
  ...
  async function handleCreateMaterial(e: React.FormEvent) {
    ...
    const res = await fetch("/api/inventario/materiales", {
      ...
      body: JSON.stringify({ ...matForm, stockMinimo: parseFloat(matForm.stockMinimo) }),
    });
    if (res.ok) {
  ...
  async function handleScanRecepcion(codigo: string) {
    ...
    if (res.ok) {
      ...
      setRecMsg({ type: "success", text: `Material encontrado: ${mat.nombre}` });
    } else {
      setRecMaterial(null);
      setRecMsg({ type: "error", text: "Material no encontrado. Registra el material primero." });
    }
  }
  ...
  async function handleRecepcion(e: React.FormEvent) {
    ...
    const res = await fetch("/api/inventario/lotes", {
      ...
      body: JSON.stringify({
        ...
        cantidad: parseFloat(recForm.cantidad),
        proveedor: recForm.proveedor,
        fechaCaducidad: recForm.fechaCaducidad,
        certificado: recForm.certificado,
      }),
    });
    if (res.ok) {
        ...
        <div className="bg-white rounded-xl border overflow-hidden">
          <table className="w-full">
            ...
            <tbody className="divide-y">
              ...
              {lotes.length === 0 && (
                <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-400">No hay lotes registrados</td></tr>
              )}
            </tbody>
          </table>
        </div>
